...
def save_images(webpage, visuals, image_path, width=256):
    ...
    for label, im_data in visuals.items():
        # ====================== ✅ 修复：自动处理列表 ======================
        if isinstance(im_data, (list, tuple)):
            # 如果是列表 [s1, s2]，逐个保存
            for idx, im_item in enumerate(im_data):
                im = util.tensor2im(im_item)
                image_name = f'{name}_{label}_{idx}.png'
                save_path = os.path.join(image_dir, image_name)
                util.save_image(im, save_path)
                ims.append(image_name)
                txts.append(f'{label}_{idx}')
